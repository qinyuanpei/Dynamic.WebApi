...
using Abp.DynamicWebApi.Migrations.SeedData;
...
namespace Abp.DynamicWebApi.Tests
{
    public abstract class DynamicWebApiTestBase : AbpIntegratedTestBase<DynamicWebApiTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected DynamicWebApiTestBase()
        {
        ...
        protected void UsingDbContext(Action<DynamicWebApiDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DynamicWebApiDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DynamicWebApiDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DynamicWebApiDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DynamicWebApiDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DynamicWebApiDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DynamicWebApiDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DynamicWebApiDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DynamicWebApiDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DynamicWebApiDbContext, Task<T>> func)
        {
            T result;
